# =============================================================================
# VSM MCP Jira Lambda - Main Infrastructure
# =============================================================================
...
resource "aws_iam_role" "lambda_execution" {
  name = "vsm-mcp-jira-lambda-${var.environment}"
...
resource "aws_iam_role_policy" "lambda_logs" {
  name = "vsm-mcp-jira-lambda-logs-${var.environment}"
...
resource "aws_iam_role_policy" "lambda_secrets" {
  name = "vsm-mcp-jira-lambda-secrets-${var.environment}"
  role = aws_iam_role.lambda_execution.id
...
resource "aws_secretsmanager_secret" "jira_credentials" {
  name        = "vsm-mcp-jira-credentials-${var.environment}"
  description = "Jira API credentials for VSM MCP Jira Lambda"
...
resource "aws_lambda_function" "mcp_jira" {
  function_name = "vsm-mcp-jira-${var.environment}"
  description   = "VSM MCP Jira Lambda - Exposes Jira MCP tools as HTTP API"
  ...
  tags = {
    Name = "vsm-mcp-jira-${var.environment}"
  }
}
...
resource "aws_cloudwatch_log_group" "lambda" {
  name              = "/aws/lambda/${aws_lambda_function.mcp_jira.function_name}"
...
resource "aws_apigatewayv2_api" "main" {
  name          = "vsm-mcp-jira-${var.environment}"
  description   = "API Gateway for VSM MCP Jira Lambda"
  protocol_type = "HTTP"
...
resource "aws_apigatewayv2_integration" "lambda" {
  ...
  integration_uri        = aws_lambda_function.mcp_jira.invoke_arn
...
resource "aws_lambda_permission" "apigw" {
  ...
  function_name = aws_lambda_function.mcp_jira.function_name
